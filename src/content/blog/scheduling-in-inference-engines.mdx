...
pubDate: 'Sep 22 2025'
coverCredit: '*Verbouwing van Damrak 70 voor de firma C.J. Boele*, ontwerptekening, [Amsterdam City Archives](https://unsplash.com/photos/a-blueprint-of-a-building-with-a-bunch-of-windows-URnyBZCnlIs).'
---
...
that the LLM on the GPU can work on efficiently[>1].

[>1]: For autoregressive LLMs (i.e. generative text models, like LLaMA, Deepseek, etc.) this 'scheduling' can be done after each generated token. Doing this scheduling at the 'token-level' has come to be called 'continuous batching'. For non-autoregressive LLMs, we're stuck with 'request-level' scheduling, usually by manually ticking a time clock, and processing all the requests in between ticks in a batch.
...
the performance of your inference system[>2].

[>2]: Probably. We can saturate the compute capacity of the accelerator without using all of the GPU memory, so adding more sequences to the batch doesn't necessarily improve the throughput. But it does increase some other useful properties.

This is a scheduling problem. We're going to have a look at how its
done, and think about how to improve it.
...
There's some pretty obvious problems with this approach. All the gray
squares are just wasted memory. VRAM is expensive, we can't go around
allocating it and not using it.

The solution that most systems have settled on is [paged
attention](/blog/paged-attention). But now we've dealt with the fragmentation
problem, we've made it possible to fit many more sequences in memory. Each
request that we want to process needs to be prefilled - which requires us to
allocate memory for each incoming token. Over time, each sequence in the batch
will grow - by one token per iteration[>3].

[>3]: More, if we do speculative decoding.
...
doesn't necessarily give you maximal performance.
...
1. It's completely first come first served[>4]. If a sequence gets preempted,

[>4]: There are a few edge cases: 1. For encoder-decoder (whisper) models, there's some logic that I've elided that will let requests with shorter encoder prompts schedule before requests with longer encoder prompts, if the higher priority requests won't fit into the encoder batch. 2. For models that have a prefix cache hit in a remote KV cache store, the scheduling algorithm will skip scheduling those requests until the KV cache transfer completes. 3. Same applies for models whose structured output FSM is being constructed.
...
2. It doesn't take into account 'temporal cache locality'. Prefix caching is a
   big deal for prefill inference. Scheduling requests with similar prefix caches
...
those sorts of inputs better[>5].

[>5]: Think pivot selection in quicksort.
...
maximum sequence length that the system will accept is $N$[>6] - i.e. a single

[>6]: You usually would pick some bound for this number much lower, partly for the reasons we'll outline.
...
which completes alone, etc.[>7]

[>7]: Apparently this is called the "Convoy effect".
...
Then, we could just do the first $10$ requests in the same batch[>8].
...
sequences will get preempted. When the sequence is preempted, we throw away[>9]
all the work we've done so far, and the sequence has to start again later.

[>9]: Or swap out, but the default setting in vLLM is to recompute.

Given a sequence of requests, how can a nefarious user order them to maximise
the amount of useless compute that the engine performs?
...
At time $0$, $8$ sequences will join the batch[>10]. At time $1$, the KV cache

[>10]: We need to give each sequence at least 1 token to grow into.
...
proceeds to the next iteration, we have to preempt the last $3$ sequences. At
time $2$, the same thing will happen again, so we cut down our batch size
again, and again, .... Once the first sequence completes, we can start the
process again with the second sequence in the first slot. Only the sequence in
the first slot ever completes.
...
Each completion of length $N$ triggers $O(N)$ preemptions. Is this
optimal?[>11]
