...
alongside a number of ongoing decodes. See the
...
1. **All the linear ops**: Softmaxes, norms, biases, SwiGLU, etc.
2. **Framework overhead**: Kernel launch overheads, data movement overheads,
   scheduling overheads, etc.
3. **Imperfect comms overlapping**: We haven't talked about comms at all -
   we've been implicitly assuming that all data transfers for an op happen
   perfectly asynchronously with compute. This definitely doesn't always happen.
4. **Chunked prefilling overheads**: There's more memory movement with chunked
...
You should be able to see that we get pretty close! Most configurations, across
most accelerators hit roughly 50% of the possible performance. It's worth
