...
pubDate: 'Sep 23 2025'
coverCredit: 'Jacob Leupold, Feuermaschine plate from [*Theatrum Machinarum Hydraulicarum*](https://www.e-rara.ch/zut/content/titleinfo/14159788) (1724–25).'
---
...
problem to make scheduling easier.
...
[>1]: L. Torvalds. The Linux Kernel Mailing List. [here](http://tech-insider.org/linux/research/2001/1215.html), Feb. 2001.
...
runs it to completion. And so on, and so on.[>2]

[>2]: This is how we do it in LLM inference [currently](/blog/scheduling-in-inference-engines).
...
Shortest remaining time scheduling has a nice property - it minimizes the
average waiting time for users.[>3] In the LLM context, this is effectively the
'average' time between tokens.

[>3]: It does this by disadvantaging users that want a lot of tokens. But once you've finished growing the pie, you've got to figure out how to divide it up optimally.
...
[disaggregated
prefill](https://docs.vllm.ai/en/v0.9.2/features/disagg_prefill.html)[>4], then we
compute the prefill query on a separate node, and then send the KV cache across
to the decode machine.

[>4]: [here](https://github.com/ai-dynamo/dynamo), [here](https://lmsys.org/blog/2025-05-05-large-scale-ep/).

Instead of prefilling `original_prompt`, we can prefill `tokens_query`[>5].

[>5]: While the appended text looks longer here, that's just because its a short example.
...
![here](/blog-images/llm-guided-scheduling/prediction_plot.png)[>_1]

[>_1]: A plot of the number of tokens that `google/gemma-3-12b-it` thinks it will generate, vs the actual number that it generates. Max number of tokens is set to $2000$. Postfix prompt is the following: `"f"\n\nDo not answer the above question. Instead, just predict: how many tokens would your response be if you were to answer? (Note: maximum response length is {MAX_TOKENS} tokens) Respond with only a number. Be precise"`
