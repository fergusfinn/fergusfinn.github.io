...
draft: false
stickyToc: true
doublewordUrl: 'https://blog.doubleword.ai/what-happens-when-you-run-a-cuda-kernel'
---
...
back up to the answer[>D].

[>D]: An aside, this post is an instance of the 'legibility transition' that
...
[>5]: These offsets are the constant bank offsets `0x160`, `0x168`, `0x170`,
...
into the closed-source user-mode driver (`libcuda.so.1`)[>6] to initiate the
...
infrastructure the driver needs to talk to the device, including the _channel_
...
CUDA 12.2, module loading is lazy by default[>7]—the driver defers uploading a
kernel's SASS cubin to the card's memory until the very first time that
specific kernel is actually launched. 

[>6]: The usermode bit of the driver comes with the GPU's kernel driver, not
...
[>7]: Controlled by `CUDA_MODULE_LOADING`. It shipped opt-in in CUDA 11.7 and
defaulted to `EAGER` for years; the 12.x series flipped the default to `LAZY`
(which can be overridden if you want loading costs paid up front).
...
comprise the _channel_ — the work queue that runs the GPU's operations.
...
[>8]: In this case, base is a GPU virtual address pointing to host memory
...
[>_1]: Where the different pieces live. 

Our launch is triggered by a burst of methods, first
[`SET_INLINE_QMD_ADDRESS_A/B`](https://github.com/NVIDIA/open-gpu-kernel-modules/blob/590.48.01/src/common/sdk/nvidia/inc/class/clc6c0.h#L403-L409)[>9]
followed by a run of [`LOAD_INLINE_QMD_DATA`](https://github.com/NVIDIA/open-gpu-kernel-modules/blob/590.48.01/src/common/sdk/nvidia/inc/class/clc6c0.h#L409-L410).
These methods serve to stream an object called the "Queue Meta Data" (**QMD**)
into the pushbuffer.

[>9]: How I know it's this method, given that `libcuda` is closed source: see
the [appendix](#appendix-how-to-look-inside-the-launch).
...
the GPU's **host engine**[>10] hasn't acted: it doesn't watch the cursor on modern
cards[>11], so the change to `GP_PUT` just sits there until something tells the
engine to look.

[>10]: The part of the GPU's control logic that interfaces with the host.
...
[>11]: They used to: older GPUs [snooped USERD](https://github.com/NVIDIA/open-gpu-kernel-modules/blob/590.48.01/src/common/unix/nvidia-push/src/nvidia-push.c#L421-L438),
...
The host engine hands the QMD to the **compute work distributor**[>12]. There is
...
[>12]: Sometimes still called the GigaThread Engine
...
RTX 4090 chip with **128 SMs**[>13]. The distributor's task is to keep all 128
saturated with work.

[>13]: NVIDIA's AD102-300-A1 SKU disables 16 of the physical 144 SMs on the full
...
Counter](https://en.wikipedia.org/wiki/Program_counter) (PC)[>14]. Schedulers on the
...
[>14]: Since Volta, the model goes finer still — each *thread* carries its own
...
lanes currently at a common PC.
...
[>15]: `cudaGetDeviceProperties` tells you this information
...
1. **Register capacity**: Each block needs $256 \times 16 = 4,096$ registers.
   On registers alone, an SM could fit $65,536 / 4,096 = 16$ resident blocks.
2. **Thread capacity**: The hardware caps each SM at 1,536 active threads.
   Divided by our block size, this yields $1,536 / 256 = 6$ resident blocks.
...
`ptxas`[>16]. These scheduling control bits dictate hardware timing directly and
...
[>16]: The clearest public reconstructions are the Citadel microbenchmarking
...
When you disassemble a binary using NVIDIA's standard `nvdisasm` tool, the raw
...
architectural concept remains identical: compile-time static scheduling
metadata is packed directly into the instruction stream to keep the SM hardware
as simple and power-efficient as possible.

</Details>

Running `cuobjdump -sass` on our `vadd`, each instruction comes with its raw
...
Pulling out the control payloads[>16] — their bit layout is in the
[appendix](#decoding-the-sass-control-words) — you can see the schedule that
`ptxas` wrote, with each directive in action:
...
barrier, `B2`. The `FADD`, the first instruction that needs the loaded `R4` and
`R3`, carries a wait on `B2`: until both loads have returned and the barrier
clears, the warp is **ineligible**, and the scheduler skips it for one of the
other eleven warps in the sub-partition.

The `FADD`→`STG` hand-off is directive 1. A floating-point add has a fixed
latency, so there is no barrier: `FADD` just carries `stall=5`, which parks the
warp for the few cycles it takes for `R9` to land before `STG` reads it. 

The yield bit, directive 2, toggles on & off  across the sequence, as the
compiler nudges scheduling priority around the operations that are about to
wait.

Each cycle the scheduler reads the warp's six-bit barrier state and a small
stall counter, and makes the eligibility decision for each warp. This is how a
GPU hides latency with close to zero hardware-scheduling overhead.
...
travel across the memory bus to the physical GDDR6X VRAM chips[>17]. The
`STG.E` store that writes `c[i]` at the end of the loop follows the exact
same path in reverse[>18].

[>18]: In principle anyway, we'll see later that `c[i]` never hits VRAM.

[>17]: The RTX 4090 uses GDDR6X memory rather than the High-Bandwidth Memory
(HBM) found in datacenter-class GPUs like the A100 or H100. 
...
So the `10.78` $\mu$s just comes down to how fast the DRAM bus can feed the
kernel its inputs, here about four-fifths of peak[>19].

[>19]: Only the two inputs cross the bus, not the full 12 MB. `ncu` shows
...
The result is now sitting in the GPU's L2 cache. The CPU is what runs our
terminal, so it needs to get the result in order to show it to us. We return to
its view of events.
...
The device-to-host `cudaMemcpy(c, dc, …)`[>20] copy sits behind the kernel on
...
Because `c` is still sitting dirty in the 72 MB L2 — the `STG.E` stores never
...
[>20]: A pinned-memory `cudaMemcpyAsync` would skip the wait and let the host
run ahead.


Once the copy finishes, it posts its _own_ semaphore, which the host was
waiting on in `cudaMemcpy`. `cudaMemcpy` completes on the host, `c` is ordinary
host memory again, and `printf` loads `c[0]` and `c[n-1]` out of RAM, formats
them into a string, and hands them to a `write` syscall on stdout. 
...
carried that result back across the bus to where `printf` was waiting, and we
learnt that:
...
you need to read the memory. We used an `LD_PRELOAD` shim that wraps `mmap`,
records every region the driver maps from the `/dev/nvidia*` file, and exposes
...
`SET_INLINE_QMD_ADDRESS_A`. The 66 words are the two address words
...
one-kernel program records 948 of them[>21], almost all on two file descriptors
...
[>21]: Almost all are one-time setup; a steady launch loop makes far fewer.

The magic byte `0x46` is `'F'`, the NVIDIA resource manager's ioctl magic[>22].
...
`0x2A` is `NV_ESC_RM_CONTROL` and `0x2B` is `NV_ESC_RM_ALLOC`. 

[>22]: The 'magic' byte is a value every NVIDIA ioctl carries as a sanity check;
...
field `ptxas` packs into the top of each instruction's second 64-bit word, which
`cuobjdump -sass` prints next to the mnemonic:
...
6-bit mask is the barriers it waits on, `Y` is the yield bit, and `stall` is
the static cycle count. The layout is undocumented and reconstructed from
microbenchmarking[>16].
